...
class TestSearchJobs:
    async def test_search_jobs_returns_empty_when_exa_unavailable(
        self, db_session,
    ):
        """Search should return empty results when Exa is not configured."""
        request = JobSearchRequest(query="python developer", platforms=[])
        with patch("app.services.job_search.ExaJobSearch") as MockExa:
    ...
    async def test_search_jobs_returns_results_from_exa(self, db_session):
        """Search should persist Exa results and return them."""
        from app.core.job_discovery.exa_search import JobListing
        ...
        with (
            patch("app.services.job_search.ExaJobSearch") as MockExa,
            patch("app.services.job_search.get_settings") as mock_settings,
        ):
            mock_settings.return_value = MagicMock(
                exa_api_key=MagicMock(get_secret_value=lambda: "fake-key")
            )
            instance = MockExa.return_value
            instance.available = True
            instance.search_jobs = AsyncMock(return_value=[mock_listing])
            result = await job_search.search_jobs(db_session, request)

        assert result.total == 1
        assert result.items[0].title == "Python Dev"
        assert result.items[0].platform == "exa"

    async def test_search_jobs_handles_exa_failure(self, db_session):
        """Search should return empty results when Exa raises."""
        request = JobSearchRequest(query="python developer", platforms=[])

        with patch("app.services.job_search.ExaJobSearch") as MockExa:
            instance = MockExa.return_value
            instance.available = True
            instance.search_jobs = AsyncMock(side_effect=RuntimeError("network error"))
            result = await job_search.search_jobs(db_session, request)
